...
class AutoFocusDataset(Dataset):
    ...
    def __init__(self, config, dataset_name, split=None):
        ...
        # 从配置文件中获取椒盐噪声的参数
        self.p_salt_pepper = config['Dataset']['transforms']['salt_pepper_noise']['p_salt_pepper']  # 新增的椒盐噪声概率
    ...
    def __getitem__(self, idx):
        ...
        depth = Image.open(self.paths_depths[idx])
        ...
        if random.random() < self.p_crop:
            ...
            segmentation = transforms.Resize((self.resize, self.resize),
                                             interpolation=transforms.InterpolationMode.NEAREST)(segmentation)
        ...
        if random.random() < self.p_rot:
            ...
            # 添加椒盐噪声
            if random.random() < self.p_salt_pepper:  # 根据椒盐噪声的概率
                image = np.array(image)  # Convert PIL image to numpy array
                salt_pepper = np.random.rand(*image.shape[:2])  # Generate random noise
                image[salt_pepper < 0.02] = 0  # Add pepper noise
                image[salt_pepper > 0.98] = 255  # Add salt noise
